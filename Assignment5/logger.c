...
void write_log(int uid, unsigned char *file_name, int access_type, int is_action_denied, char *digest)
{
	time_t t = time(NULL);
	struct tm tm;
	tm = *localtime(&t);
	...
	if (ptr != NULL)
	{
		fprintf(ptr, "%d\t%s\t%02d/%02d/%d\t%02d:%02d:%02d\t%d\t%d\t%s\n", uid, file_name, tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900, tm.tm_hour, tm.tm_min, tm.tm_sec, access_type, is_action_denied, digest);
	}
...
fopen(const char *path, const char *mode)
{
	...
	/* Call log */
	write_log(uid, file_name, file_mode, is_action_denied, md5_hash);
...
fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream)
{
	...
	write_log(uid, path, 2, is_action_denied, md5_hash);
